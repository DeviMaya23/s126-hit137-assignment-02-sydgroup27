def compare(raw_file: str, decrypt_file: str) -> None:
    ...
    try:
        ...
        if len(raw) != len(decpt):
            print(
                f"There is a difference in length of the files. "
                f"{raw_file} is {len(raw)} and {decrypt_file} is {len(decpt)}")

        min_len = min(len(raw), len(decpt))
        for i in range(min_len):
            if raw[i] != decpt[i]:
                print(
                    f"There is a difference in character "
                    f"{raw[i]} in {raw_file} and {decpt[i]} in {decrypt_file}")
